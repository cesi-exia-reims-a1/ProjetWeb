...
$Centre_Personne = (isset($_POST['Centre_Personne'])) ? ($_POST['Centre_Personne']) : NULL;
...
else
{
	...
	if (preg_match('#^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{6,}$#', $Mot_De_Passe)){
		$requete = $bdd->prepare("INSERT INTO personne ( Nom_Personne, Prenom_Personne, Adresse_eMail, Mot_De_Passe, Centre_Personne) VALUES ( :Nom_Personne, :Prenom_Personne, :Adresse_eMail, :Mot_De_Passe, :Centre_Personne)"); 
	 // Liaison des variables de la requête préparée aux variables PHP 
		...
		$requete->bindValue(':Centre_Personne', $Centre_Personne, PDO::PARAM_STR);
	// Exécution de la requête 
		$requete->execute();
