...
if(isset($_POST['categories']) && $_POST['categories'] != 'no-cat' && isset($_POST['prix']) && $_POST['prix'] == 'asc'){//si on a un prix croissant + un filtre de catégorie
	$requete = $bdd->prepare("SELECT * FROM produit WHERE Categorie_Produit = :categorie AND Prix_Produit > :prixAsc AND Supprime = 0 ORDER BY Prix_Produit ASC");
	...
	$requete = $bdd->prepare("SELECT * FROM produit WHERE Categorie_Produit = :categorie AND Prix_Produit > :prixDesc AND Supprime = 0 ORDER BY Prix_Produit DESC");
	$requete->bindParam('categorie', $_POST['categories'], PDO::PARAM_STR);
	...
	$requete = $bdd->prepare("SELECT * FROM produit WHERE Supprime = 0 ORDER BY Prix_Produit ASC");
	$requete->bindParam('prixAsc', $_POST['prix'], PDO::PARAM_STR);
	$requete->execute();
	$requete->execute();
}
else if(isset($_POST['prix']) && $_POST['prix'] == 'desc'){//si on a que un filtre de prix décroissant
	$requete = $bdd->prepare("SELECT * FROM produit WHERE Supprime = 0 ORDER BY Prix_Produit DESC");
	$requete->bindParam('prixDesc', $_POST['prix'], PDO::PARAM_STR);
	$requete->execute();
	$requete->execute();
}
else if(isset($_POST['categories']) && $_POST['categories'] != 'no-cat'){//Si on a que un filtre de catégorie
	$requete = $bdd->prepare("SELECT * FROM produit WHERE Categorie_Produit = :categorie AND Supprime = 0");
	$requete->bindParam('categorie', $_POST['categories'], PDO::PARAM_STR);
	$requete->execute();
} else{
	$requete = $bdd->prepare("SELECT * FROM produit WHERE Supprime = 0");
	$requete->execute();
}
