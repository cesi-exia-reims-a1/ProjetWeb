...
$Recursif =(isset($_POST['Recursif'])) ? ($_POST['Recursif']) : NULL; $URL_PHoto =(isset($_POST['URL_PHoto'])) ? ($_POST['URL_PHoto']) : NULL; $Description_Evenement =(isset($_POST['Description_Evenement'])) ? ($_POST['Description_Evenement']) : NULL;
...
else
{
	...
	// Requête préparée pour empêcher les injections SQL 
		...
		$requete = $bdd->prepare("INSERT INTO evenement ( Nom_Evenement, Date_Evenement, Payant, Prix, Recursif, URL_PHoto, Description_Evenement) VALUES ( :Nom_Evenement, :Date_Evenement, :Payant, :Prix, :Recursif, :URL_PHoto, :Description_Evenement)"); 
	 // Liaison des variables de la requête préparée aux variables PHP 
	var_dump($requete);
		$requete->bindValue(':Nom_Evenement', $Nom_Evenement, PDO::PARAM_STR); 
